...
if __name__ == "__main__":
    ...
    print(f"Cost: {test_alns.calculate_obj(initial_solution):.2f}")
    print(
        f"Feasible: {FeasibilityChecker.check_solution(initial_solution)}")
    print('-----------------------------')
    ...
    for i in range(ITER_NUM):
        old_cost = test_alns.calculate_obj(solution1)
        solution1 = test_alns.iterate(solution1)
        new_cost = test_alns.calculate_obj(solution1)
    ...
    print('-----------------------------')
    print(f"Final cost: {test_alns.calculate_obj(solution1):.2f}")
    print(
        f"Final feasible: {FeasibilityChecker.check_solution(solution1)}")
